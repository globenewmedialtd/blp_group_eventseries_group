...
namespace Drupal\blp_group_eventseries_group\Subscriber;
 ...
 * Class Route.
 *
 * @package Drupal\blp_group_eventseries_group\Subscriber
 */
  ...
  protected function alterRoutes(RouteCollection $collection) {
    ...
    foreach ($join_routes as $name => $argument) {
      if ($route = $collection->get($name)) {
        ...
        $requirements = array_merge($current, ['_eventseries_group_join_permission' => $argument]);
    ...
    \Drupal::moduleHandler()
      ->alter('blp_group_eventseries_group_content_routes', $content_routes);

    foreach ($content_routes as $name) {
      if ($route = $collection->get($name)) {
        $current = $route->getRequirements();
        $requirements = array_merge($current, ['_eventseries_group_content_visibility' => 'public']);
        $route->addRequirements($requirements);
      }
    }
  }
