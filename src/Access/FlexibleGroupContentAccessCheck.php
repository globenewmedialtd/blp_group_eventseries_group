...
namespace Drupal\blp_group_eventseries_group\Access;
...
/**
 * Determines access to routes based eventseries_group membership and settings.
 */
class FlexibleGroupContentAccessCheck implements AccessInterface {
  ...
  public function access(Route $route, RouteMatchInterface $route_match, AccountInterface $account) {
    $permission = $route->getRequirement('_eventseries_group_content_visibility');
    ...
    if ($type instanceof GroupTypeInterface && $type->id() !== 'es_group') {
      return AccessResult::allowed();
    }
    ...
    if (!$account->isAuthenticated() && !blp_group_eventseries_group_public_enabled($group)) {
    ...
    if ($account->isAuthenticated() && !blp_group_eventseries_group_community_enabled($group)
      && !blp_group_eventseries_group_public_enabled($group)
